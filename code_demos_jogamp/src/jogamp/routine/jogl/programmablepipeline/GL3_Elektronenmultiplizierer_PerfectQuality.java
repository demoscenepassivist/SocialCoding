 ...
 **  \/____.                                                                             .____\/     
 **
 ** This is the high quality version of my intro port! Changes: Removed the intermediate FBO and 
 ** raymarched every bitmap orbit-trap pixel perfectly, 4x super sampling, increased raymarching steps
 ** to remove remaining artefacts from the starter scene :)
 **
...
public class GL3_Elektronenmultiplizierer_PerfectQuality extends BaseRoutineAdapter implements BaseRoutineInterface,BaseFrameBufferObjectRendererInterface {
    ...
    public void initRoutine(GL2 inGL,GLU inGLU,GLUT inGLUT) {
        ...
        mBaseSuperSamplingFBOWrapper = new BaseSuperSamplingFBOWrapper(4.0f,this);
    ...
    public void init_FBORenderer(GL2 inGL,GLU inGLU,GLUT inGLUT) {
        int tFragmentShader = ShaderUtils.loadFragmentShaderFromFile(inGL,"/shaders/raymarchingshaders/elektronenmultiplizierer_perfectquality.fs");
        mLinkedShaderID = ShaderUtils.generateSimple_1xFS_ShaderProgramm(inGL,tFragmentShader);
        mScreenDimensionUniform2fv = DirectBufferUtils.createDirectFloatBuffer(new float[] {(float)mBaseSuperSamplingFBOWrapper.getSuperSampledWidth(), (float)mBaseSuperSamplingFBOWrapper.getSuperSampledHeight()});
    }
    ...
    public void mainLoop_FBORenderer(int inFrameNumber,GL2 inGL,GLU inGLU,GLUT inGLUT) {
        ...
        //use for realime rendering ... but i doubt this will be possible in the next decade X-)
