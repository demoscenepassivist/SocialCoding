...
vec3 ss(vec2 offsets) {
   ...
   if (temp_scene_number==7) {
      ...
      color += rd(custom_glFragCoord, eyeoffset*1.5)*0.8;
      as(7,temp_scenetimer);      
      //to remove background artifacts
      color *= clamp(rd(custom_glFragCoord, eyeoffset),0.15,1.0)*7;
   } else if (temp_scene_number==5) {
      as(99,temp_scenetimer);
      fractaliterations = 13;
      color += rd(custom_glFragCoord, eyeoffset*1.5)*1.0;
      as(5,temp_scenetimer);      
      //to remove background artifacts
      color *= clamp(rd(custom_glFragCoord, eyeoffset),0.05,1.0)*6;
   } else {
      color = rd(custom_glFragCoord, eyeoffset);
   }
   return color;
}
