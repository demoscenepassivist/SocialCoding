...
//!P center scale offset ...
    vec3 offset;
    if (en==6) {
        offset = vec3(0.61,0.1*et_internal,0.99);
    } else {
...
//!P max number of raymarching steps (190);
    for (int i = 0; i < 190; i++) {
        steps = i;
...
vec4 raymarch_orbittrap_image(vec2 fragcoord) {
    ...
    if (en==2 || en==0) {
        ...
        position = vec2(fragcoord.x*sizedode.x,fragcoord.y*sizedode.y);
    } 
    if (en==3) {
        camerapositiondode = vec3(0.0,0.0,-2.7*(10.-et_internal));
    } 
    if (en==4) {
        camerapositiondode = vec3(0.0,0.0,-2.7*(10.-9.3));
        fractalplanex_var = et_internal;
    ...
    if (en==5) {
        ...
        fractalplaney_var = -1.0-et_internal;
    }
    if (en==6) {
        camerapositiondode = vec3(0.0,0.0,-2.7*(10.-9.5));
        fractalplanex_var = et_internal;
        fractalplaney_var = sin(et_internal*0.03)-1.0;   
    }
    if (en==7 || en==1) {
        sizedode = vec2(384,384);
        et_internal = 9.1;
        fractalplanex_var = et_internal;
        fractalplaney_var = sin(et_internal*0.93)-1.0;
        camerapositiondode = vec3(0.0,0.0,-2.7);
        position = vec2(fragcoord.x*sizedode.x,fragcoord.y*sizedode.y);
    } 
...
//!P orbit trap scale and offset    
    ...
    vec4 s = raymarch_orbittrap_image(sp);
    if (s.a > 0.0) {
        c = mix(c, s, s.a);
    }
    return c;
...
//!P use offset-julia from 2.25 to 2.5
        c = vec2(sin(et_internal+2.07)*0.05,cos(et_internal+2.07));
    } else {
        //mandelbrot mode ...
        c = z;
    }
...
//!P camera position for julia ...
        ...
        if (en==0) {
            //julia
            camerapositionjulia = vec3(-0.2,-0.515,0.095347+(et_internal*1.75));
        } else {
            //mandelbrot
            camerapositionjulia = vec3(0.325895,0.049551,0.0005+et_internal);
        }
//!P absolute output size of julia orbit trap ...
    ...
    if (en==2 || en==7) {
        ...
        color_tv *= 0.9+0.1*sin(1.5*tm+position.y*750.0);
        //tv flicker effect
