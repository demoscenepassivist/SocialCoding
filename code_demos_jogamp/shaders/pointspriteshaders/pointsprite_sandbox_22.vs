...
void main(void) {
    ...
    float rings = 50.0*sin(time*0.02);
    vec2 position = -1.0+2.0*gl_TexCoord[0].st;
    position.x *= 1.77;
    vec2 positionoffset1 = vec2(cos(time*0.043)*3.0,sin(time*0.083))*1.5;
    vec2 positionoffset2 = vec2(sin(time*0.08),cos(time*0.063))*1.5;
    ...
    vec4 value2 =  vec4(sin(ang2+rings*rad2+speed*(time*0.5)));
    value2 = vec4(0.5,0.5,0.5,0.5)*(vec4(1.0,1.0,1.0,1.0)+value2);
    vec4 color2 = vec4(circleradius1/5.0,(circleradius2/5.0+circleradius1/5.0)/1.5,circleradius1/5.0,1.0);
    if(spiralpos2.x<0.0) {   
        value2 = 1.0-value2;
    }
    frag_color = value1*color1*value2*color2;//+value2;
    
    float gray = dot(frag_color.rgb, vec3(0.299, 0.587, 0.114)); 
    vec4 raw_vertex = gl_Vertex;
    raw_vertex.z -= min(gray*40.0,40.0);      
