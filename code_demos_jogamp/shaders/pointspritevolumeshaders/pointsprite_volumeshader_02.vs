...
void main( void ) {
   vec3 position = vec3(-1.0+gl_MultiTexCoord0.xyz*2.0);
   position.z += time;
   float n = surface3(position*mat3(1,0,0,0,.8,.6,0,-.6,.8),1.0);
   float n2 = surface3(position*mat3(1,0,0,0,.8,.6,0,-.6,.8),.5);
   float lum = length(n);
   ...
   color = vec4(curr_color.x,curr_color.y, curr_color.z, 1.0);
